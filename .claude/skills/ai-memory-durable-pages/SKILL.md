...
When the user explicitly wants a note to be temporary, pass `expires_at` as an RFC3339 instant or a bare `YYYY-MM-DD` (the end of that day in UTC). After the TTL, normal search, recent, and briefing reads hide the page; the next forget sweep deletes it. A TTL outranks `pinned`, so do not combine them unless the user has deliberately requested that behavior.
